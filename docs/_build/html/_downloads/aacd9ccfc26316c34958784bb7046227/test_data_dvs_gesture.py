...
def test_import_module_without_optional_deps(monkeypatch):
    ...
    monkeypatch.delitem(sys.modules, "dch_data.dvs_gesture", raising=False)

    mod = _reload_module("dch_data.dvs_gesture")
    assert hasattr(mod, "DvsGestureLoader")
...
def test_loader_raises_when_tonic_missing_on_use(monkeypatch, tmp_path):
    ...
    loader = mod.DvsGestureLoader(root=str(tmp_path), split="train", download=False)
...
def test_numpy_missing_on_use_path(monkeypatch, tmp_path):
    ...
    tonic_stub.datasets = types.SimpleNamespace(DVSGesture=_DVSGestureStub)
