...
# License: MIT
"""
DVS Gesture dataset loader with optional dependency gating (tonic, numpy).
...
Public API
- DvsGestureLoader: minimal loader with lazy imports and load_one(index) method.

Usage
    loader = DvsGestureLoader(root="~/.cache/tonic", split="train", download=False)
...
def _lazy_import(module_name: str, context: str) -> Any:
    ...
    Args:
        ...
        context: Human-friendly context for the error message (e.g., "DVS Gesture loader").
...
class DvsGestureLoader:
    """
    Minimal DVS Gesture loader that constructs the tonic dataset on demand and loads a single item.
    ...
    Methods:
        load_one(index: int) - returns dataset[index] from tonic.datasets.DVSGesture
    ...
    def load_one(self, index: int) -> Any:
        """
        Load a single sample by index using tonic.datasets.DVSGesture.
        ...
        Returns:
            Whatever tonic.datasets.DVSGesture returns for dataset[index].

        Raises:
            ImportError: If 'tonic' or 'numpy' is not available.
        """
        # Lazy-gate optional deps with actionable messages
        tonic = _lazy_import("tonic", "DVS Gesture loader")
        _ = _lazy_import("numpy", "DVS Gesture loader")

        ds = tonic.datasets.DVSGesture(
